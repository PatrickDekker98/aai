...
"""
return all eucledean distances off all points
"""
...
def forAllDatesDetermineSeason(days, data, labels, K):
    for day in days:
        deys = getAllEuclideanDistancesOfOnePoint(day, data)
        printLabel(labels, deys[:K])

data = dataReader('dataset1.csv') 
validation = dataReader('validation1.csv') 
days = dataReader('days.csv')

minMaxList = findMinMax(data)
